...
export default function InvitationCard({ card }: InvitationCardProp) {
    ...
    return (
        <Link to={link}>
            <div className='w-full flex rounded-lg my-2 h-40 relative transition duration-300 hover:-translate-y-1 cursor-pointer '
            ...
            >
                ...
                <div className='absolute w-full h-full flex justify-center items-center -top-[32px] '>
                ...
                <div className='grid grid-cols-6 grid-rows-2 justify-start overflow-hidden items-center w-full h-full'>
                    <div className='flex flex-col min-w-full items-end justify-end col-span-5 row-span-2'>
                        <div className={`flex flex-col h-full justify-end items-start w-full p-4 relative -bottom-14 transition duration-300 ${hover ? 'md:-translate-y-16 lg:-translate-y-12' : ''}`}>
                            <h2 className='text-2xl font-semibold text-gray-200'>{title}</h2>
                            <h3 className='absolute w-3/5 md:w-2/4 lg:w-3/4 top-12 mt-1 font-thin hidden md:block text-sm text-gray-200'>{description}</h3>
                        </div>
                    </div>
