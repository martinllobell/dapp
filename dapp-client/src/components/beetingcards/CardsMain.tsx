...
export const CardsMain: FC<CardsMainProps> = ({ darkMode, filter }) => {
  ...
  const fetchEventData = async (eventID: number) => {
    try {
      ...
      const response = await axios.get(
        `/api/sportsdata/v3/nba/scores/json/Games/${season}`,
        {
          headers: {
            "Ocp-Apim-Subscription-Key": "01e1ae2db9b642229876fbd8527f4822",
          },
        }
      );
  ...
  useEffect(() => {
    const fetchBets = async () => {
      ...
      try {
        const numberOfBets = await contracts.p2pBetting.methods
          .getNumberOfBets()
          .call();
        const bets: Bet[] = [];
        ...
        for (let i = 0; i < numberOfBets; i++) {
          ...
          if (bet) {
            ...
            const newBet: Bet = {
              ...
              winCondition:
                String(bet.betData[0]) === "0"
                  ? "Final result"
                  : `Goals/Points`,
              points: bet.betData[3] ? String(bet.betData[3]) : "",
              comparator:
                bet.betData[2] !== undefined
                  ? Number(bet.betData[2]) === 0
                    ? "Exactly"
                    : Number(bet.betData[2]) === 1
                    ? "Greater than"
                    : "Less than"
                  : "",
              maxBet: bet[3]
                ? `${web3.utils.fromWei(bet[3].toString(), "ether")} ETH`
                : "0 ETH",
