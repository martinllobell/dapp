...
const WalletConnect = ({ darkMode }) => {
  ...
  const connectWallet = async (address, storedSignature) => {
    try {
      ...
      setMoney(
        parseFloat(etherBalance).toFixed(4) + " " + getCryptoName(network)
      );
    } catch (error) {
  ...
  return (
    <>
      {isConnected ? (
                ...
                <Menu.Item>
                  <div className="relative">
                    <button
                      ...
                      className={classNames(
                        "text-left w-full px-4 py-2 text-sm flex justify-between",
        ...
        <button
          onClick={open}
          className="bg-gray-200 dark:bg-gray-700 text-black dark:text-white hover:bg-gray-300 dark:hover:bg-gray-600 backdrop-blur-xl drop-shadow-xl flex items-center justify-center text-nowrap font-semibold text-sm shadow-sm shadow-black/10 w-[10rem] h-[2.3rem] rounded-lg transition ease-in-out hover:scale-105 duration-100"
        >
          <svg
            xmlns="http://www.w3.org/2000/svg"
            fill="none"
